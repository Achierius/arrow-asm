...
private:
  /// @brief The rvalue that is currently on the top of the stack, or nullopt
  /// for unknown/nothing/not an rvalue
...
public:
  ...
  void assign_top(ast::ArgNode const &arg) { top_list.push_front(arg); }
  void pop_top() { top_list.pop_front(); }
  ...
  bool same_as_top(ast::ArgNode const &arg) const {
    if (!top_list.empty()) {
      ...
      if (std::holds_alternative<ast::ImmediateNode>(topref) &&
        ...
        return top_r.category == arg_r.category &&
               top_r.register_id == arg_r.register_id;
      }
    }
    return false;
  }
...
void EmitNoParam(bytecode::BytecodeChunk &chunk, bytecode::Opcode opcode) {
  Emit(chunk, bytecode::Instruction{.opcode = opcode, .param = 0});
}
...
bool equal_type(FuncInstContext const &ctx, ast::ArgNode const &arg1,
                ast::ArgNode const &arg2) {
...
void EmitArg(bytecode::BytecodeChunk &chunk, FuncInstContext &ctx,
             ast::ArgNode const &arg) {
  // ASSUME: We should have already checked our arg type to ensure it works with
  // our instruction
  if (ctx.same_as_top(arg)) {
    // We can emit a dup!
    Emit(chunk,
         bytecode::Instruction{.opcode = bytecode::Opcode::kDup, .param = 0});
  } else {
    ...
    if (std::holds_alternative<ast::ImmediateNode>(arg)) {
      ...
      if (std::holds_alternative<int64_t>(imm)) {
        ...
        Emit(chunk,
             bytecode::Instruction{.opcode = bytecode::Opcode::kConstant,
...
void EmitIf(bytecode::BytecodeExecutable &exe, bytecode::BytecodeChunk &chunk,
  ...
  std::copy(ctx.reg_types.begin(), ctx.reg_types.end(),
            nested_ctx.reg_types.begin());
  if (condition) {
    // First, we handle our if condition
    EmitArg(chunk, ctx, *condition);
    Emit(chunk,
         bytecode::Instruction{.opcode = bytecode::kLogicalNeg, .param = 0});
...
void EmitWhile(bytecode::BytecodeExecutable &exe,
  ...
  Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kJump,
                                    .param = (int8_t)header_offset});

  // We can go back now to our first and modify the param
  chunk.code[conditional_jump].param =
      static_cast<int8_t>(chunk.code.size() - conditional_jump);
}
...
void HandleInstruction(bytecode::BytecodeExecutable &exe,
  ...
  if (std::holds_alternative<ast::IfNode>(instr)) {
    ...
    // We keep our context, but we need a new one (so we don't taint our
    // existing one) per if/elif/else branch. Then, at the end, we can union all
    // of these contexts together to continue to get the best possible type
    // checking we can
    std::vector<FuncInstContext> contexts;
    ...
    Emit(chunk,
         bytecode::Instruction{.opcode = bytecode::Opcode::kNop, .param = 0});
  ...
  if (std::holds_alternative<ast::WhileNode>(instr)) {
    ...
    case ast::NoArgOperator::kTrap: {
      // Easy.
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kTrap,
                                        .param = 0});
      break;
    }
    ...
    case ast::NoArgOperator::kRet: {
      ...
      // all of our return values. They would come from all of our prior p and
      // rr writes Alternative, we push all of our return writes to the VERY
      // bottom of our stack and then we pop all of our parameters here before
      // we ret
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kReturn,
                                        .param = 0});
      break;
    }
    }
    ...
    case ast::NoRetOperator::kPrint: {
      ...
      ctx.pop_top();
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kPrintLong,
                                        .param = 0});
      break;
    }
    case ast::NoRetOperator::kPrintChar: {
      ...
      // TODO: type checking
      ctx.pop_top();
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kPrintChar,
                                        .param = 0});
      break;
    }
    case ast::NoRetOperator::kExit: {
      ...
      // TODO: type checking
      ctx.pop_top();
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kExit,
                                        .param = 0});
      break;
    }
    default:
      AASM_COMPILE_ERROR("ERROR: No return node!", node);
      break;
    }
    ...
    case ast::UnaryOperator::kLNeg:
      ...
      break;
    default:
      AASM_COMPILE_ERROR("ERROR: unsupported unary instruction", node);
      break;
    }
    ...
    Emit(chunk, bytecode::Instruction{
                    .opcode = bytecode::Opcode::kStoreAuxiliary, .param = dst});
    ...
    if (!equal_type(ctx, arg1, arg2)) {
      AASM_COMPILE_ERROR("ERROR: nonequivalent argument types", node);
    }
    ...
    case ast::BinaryOperator::kOr:
      ...
      break;
    default:
      AASM_COMPILE_ERROR("ERROR: Unsupported binary instruction", node);
      break;
    }
    // Pop both arguments from our 'top' stack
    ctx.pop_top();
    ...
    if (itr == exe.symbol_table.end()) {
      AASM_COMPILE_ERROR("ERROR: identifier could not be found", call);
    } else {
      if (std::holds_alternative<bytecode::ChunkId>(itr->second)) {
        auto chunkId = std::get<bytecode::ChunkId>(itr->second).idx;
        Emit(chunk,
             bytecode::Instruction{.opcode = bytecode::Opcode::kCall,
                                   .param = static_cast<int8_t>(chunkId)});
      } else {
        AASM_COMPILE_ERROR("ERROR: Identifier is not a function", call);
      }
    }
    ...
    if (std::holds_alternative<ast::RValueNode>(arrow.rhs)) {
      ...
      Emit(chunk,
           bytecode::Instruction{.opcode = bytecode::Opcode::kMoveAuxiliary,
      ...
      if (itr == exe.symbol_table.end()) {
        AASM_COMPILE_ERROR("ERROR: field symbol not found", rhs);
      } else {
        if (std::holds_alternative<bytecode::FieldId>(itr->second)) {
          ...
          Emit(chunk,
               bytecode::Instruction{.opcode = bytecode::Opcode::kLoadAuxiliary,
          ...
          AASM_COMPILE_ERROR("ERROR: symbol is not a field", rhs);
        }
      }
      ...
      if (std::holds_alternative<ast::IdNode>(type)) {
        ...
        if (itr == exe.symbol_table.end()) {
          AASM_COMPILE_ERROR("ERROR: type symbol not found", id);
        } else {
          if (std::holds_alternative<bytecode::ClassId>(itr->second)) {
            ...
            Emit(chunk,
                 bytecode::Instruction{.opcode = bytecode::Opcode::kAllocateImm,
            ...
            Emit(chunk,
                 bytecode::Instruction{
                     ...
                     .param = TranslateRegister(ast::RegisterNode{
                         ...
                         .register_id = 0})});
            // 2. Call constructor
            Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kCall,
                                              .param = static_cast<int8_t>(
                                                  klass.ctor_chunk.idx)});
            ...
            AASM_COMPILE_ERROR("ERROR: symbol is not a type", id);
          }
        }
      } else {
        ...
        if (itr != exe.symbol_table.end()) {
          ...
          if (std::holds_alternative<bytecode::ClassId>(itr->second)) {
            ...
            AASM_COMPILE_ERROR("ERROR: symbol table does not have this primive "
                               "(name collision?)",
                               type);
          }
        } else {
          ...
          if (std::holds_alternative<ast::LongNode>(type)) {
            klass.fields.push_back(
                bytecode::BytecodeExecutable::ValueType::kLong);
          } else if (std::holds_alternative<ast::DoubleNode>(type)) {
            klass.fields.push_back(
                bytecode::BytecodeExecutable::ValueType::kDouble);
          } else if (std::holds_alternative<ast::PtrNode>(type)) {
            klass.fields.push_back(
                bytecode::BytecodeExecutable::ValueType::kDataPtr);
          } else {
            AASM_COMPILE_ERROR("ERROR: impossible internal compiler state!",
                               type);
          }
        ...
        // Allocate
        Emit(chunk, bytecode::Instruction{
                        .opcode = bytecode::Opcode::kAllocateImm, .param = sz});
        // Pointer is now on stack, done!
        // Note: The (implicit) dtor for a boxed pointer does not actually
        // destroy this is a potential problem.
        // TODO: See above
      }
    } else {
      AASM_COMPILE_ERROR("ERROR: unsupported arrow rhs", arrow);
    }
    ...
    if (std::holds_alternative<ast::LValueNode>(arrow.lhs)) {
      ...
      // This involves us loading it first (more specifically, MOVING it to
      // stack)
      ...
      Emit(chunk, bytecode::Instruction{.opcode = bytecode::Opcode::kDestroy,
                                        .param = 0});
      Emit(chunk,
           bytecode::Instruction{.opcode = bytecode::Opcode::kStoreAuxiliary,
                                 .param = TranslateRegister(lhs)});
      ...
      auto itr = exe.symbol_table.find(symbol);
      if (itr == exe.symbol_table.end()) {
        AASM_COMPILE_ERROR("ERROR: field symbol not found", lhs);
      } else {
        if (std::holds_alternative<bytecode::FieldId>(itr->second)) {
          auto fid = std::get<bytecode::FieldId>(itr->second).idx;
          // Load the pointer on stack
          ...
          AASM_COMPILE_ERROR("ERROR: field identifier is not a field", lhs);
        }
      }
    }
  } else {
    AASM_COMPILE_ERROR("ERROR: unsupported instruction", instr);
  }
}
...
bytecode::BytecodeExecutable ast::LowerAst(const ProgramNode &ast) {
  ...
  exe.chunks.push_back(bytecode::BytecodeChunk{.code{{bytecode::Instruction{
      .opcode = bytecode::Opcode::kReturn, .param = 0}}}});
  ...
  for (auto const &stmt : ast.statements) {
    if (std::holds_alternative<ast::FunctionNode>(*stmt)) {
      ...
      if (primitives.find(func.id.id) != primitives.end()) {
        AASM_COMPILE_ERROR("ERROR: cannot redefine primitive types", func);
      }
      if (exe.symbol_table.find(func.id.id) != exe.symbol_table.end()) {
        AASM_COMPILE_ERROR("ERROR: duplicate function definition", func);
      }
      exe.symbol_table.insert(
          {func.id.id, bytecode::ChunkId{.idx = chunkId++}});
      ...
      for (auto const &f : type.fields) {
        ...
        exe.symbol_table.insert(
            {type.id.id + ':' + f->id.id, bytecode::FieldId{fOffset++}});
        if (std::holds_alternative<LongNode>(f->type)) {
          ...
          AASM_COMPILE_ERROR("ERROR: unknown field type", type);
        }
      }
      if (primitives.find(type.id.id) != primitives.end()) {
        AASM_COMPILE_ERROR("ERROR: cannot redefine primitive types", type);
      }
      if (type_method_map.find(type.id.id) != type_method_map.end()) {
        AASM_COMPILE_ERROR("ERROR: duplicate type definition", type);
      }
      type_method_map.insert({type.id.id, {cidx, didx}});
      if (exe.symbol_table.find(type.id.id) != exe.symbol_table.end()) {
        AASM_COMPILE_ERROR("ERROR: name collision", type);
      }
  ...
  for (auto &stmt : ast.statements) {
    if (std::holds_alternative<ast::FunctionNode>(*stmt)) {
      ...
      auto &chunk =
          exe.chunks[std::get<bytecode::ChunkId>(exe.symbol_table[symbol.id])
                         .idx];
      FuncInstContext ctx{};
      ...
      if (type.ctor) {
        ...
        for (int i = 0; i < func.params.size(); ++i) {
          ctx.reg_types[TranslateRegister(ast::RegisterNode{
              .category = ast::RegisterCategory::Param,
              .register_id = static_cast<uint8_t>(i + 1)})] = *func.params[i];
        }
        ctx.reg_types[TranslateRegister(
            ast::RegisterNode{.category = ast::RegisterCategory::Param,
