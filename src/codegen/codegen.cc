...
popTwo(std::vector<llvm::Value *> &stack) {
...
private:
  ...
  [[maybe_unused]] Function *printfFn;
...
void Codegen::emitFunction(Function *f, BytecodeChunk &chunk,
                           Address &address) {
...
GlobalVariable *Codegen::createConstantStr(std::string_view str) {
  ...
  // Const address space, I couldn't find an enum outside of in the specific
  // target headers
  ...
  auto global =
      new GlobalVariable(*module, arrTy, true, GlobalValue::InternalLinkage,
                         arr, "", nullptr, GlobalVariable::NotThreadLocal, 4);
  return global;
}
...
Codegen::Codegen(BytecodeExecutable &bytecode) : bytecode{bytecode} {
  ...
  [[maybe_unused]] auto *i8 = Type::getInt8Ty(*context);
